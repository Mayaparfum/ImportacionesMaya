...
const money = (n) => new Intl.NumberFormat("es-MX",{style:"currency",currency:"MXN",minimumFractionDigits:0,maximumFractionDigits:0}).format(Math.round(n));
const DISCOUNT_5_MIN = 200;
const DISCOUNT_10_MIN = 500;
  ...
  productGrid.innerHTML=filtered.map(p=>`
    <article class="product-card">
      ...
      <div class="product-body">
        ...
        <span class="product-meta">Venta por ${p.soldBy}</span>
...
function totals(){
  ...
  let rate=0;if(subtotal>=DISCOUNT_10_MIN) rate=0.10; else if(subtotal>=DISCOUNT_5_MIN) rate=0.05;
...
function renderCart(){
  ...
  document.getElementById("discountLabel").textContent=t.subtotal>=DISCOUNT_10_MIN?"Descuento regreso a clases (10%)":t.subtotal>=DISCOUNT_5_MIN?"Descuento regreso a clases (5%)":"Descuento regreso a clases";
  ...
  const pct=Math.min(100,(t.subtotal/DISCOUNT_10_MIN)*100);
  $("#discountProgress").style.width=pct+"%";
  $("#discountMessage").textContent=t.subtotal>=DISCOUNT_10_MIN
    ? "¡Descuento del 10% aplicado automáticamente!"
    : t.subtotal>=DISCOUNT_5_MIN
      ? `¡Descuento del 5% aplicado! Agrega ${money(DISCOUNT_10_MIN-t.subtotal)} más para obtener el 10%.`
      : `Te faltan ${money(DISCOUNT_5_MIN-t.subtotal)} para obtener 5% de descuento.`;
}
...
function whatsappMessage(data){
  ...
  if(t.discount>0)lines.push(`${t.discount>0?(t.subtotal>=DISCOUNT_10_MIN?"Descuento regreso a clases (10%)":"Descuento regreso a clases (5%)"):"Descuento regreso a clases"}: -${money(t.discount)}`);
